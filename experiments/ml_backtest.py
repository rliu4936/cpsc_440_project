...
class MLStrategy(bt.Strategy):
    def __init__(self, model, feature_data):
        ...
        self.prediction_counts = {1: 0, 0: 0, -1: 0}
    ...
    def next(self):
        ...
        if self.prediction_counter % 20 == 0:
            ...
            for k, v in self.prediction_counts.items():
                ...
                label = {1: "Buy (+1)", 0: "Hold (0)", -1: "Sell (-1)"}[k]
        ...
        if not self.position:
            ...
            if self.position.size > 0 and prediction != 1:
                self.close()
...
def main():
    ...
    print(pd.DataFrame(cm, index=["True 1", "True 0", "True -1"], columns=["Pred 1", "Pred 0", "Pred -1"]))

    print("\nClassification Report:")
    print(classification_report(y_test, y_pred, labels=[1, 0, -1], target_names=["1 (buy)", "0 (hold)", "-1 (sell)"]))
